...
async function run({ foreign, home, users }) {
  ...
  const receipt1 = await foreign.relayToken(token, id, { to: users[1] })
  const relayTxHash1 = await home.waitUntilProcessed(receipt1)
  const bridgedToken = await home.getBridgedToken(token)

  await home.checkTransfer(relayTxHash1, bridgedToken, ZERO_ADDRESS, users[1], id)

  console.log('\nSending token to the Home Mediator')
  const receipt2 = await home.relayToken(bridgedToken, id, { to: users[0], from: users[1] })
