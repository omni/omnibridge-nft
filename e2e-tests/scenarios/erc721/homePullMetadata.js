...
async function run({ home, foreign, users, owner }) {
  ...
  await home.waitUntilProcessed(await foreign.relayTokenERC721(foreign.erc721Token, id))
  const bridgedToken = await home.getBridgedTokenERC721(foreign.erc721Token)

  await foreign.erc721Token.methods.setOwner(users[0]).send()
  await foreign.erc721Token.methods.setTokenURI(id, NEW_EXAMPLE_URI).send()
  ...
  assert.ok(!!relayTxHash2, 'Information request should have been executed')
  assert.ok(!relayTxHash3, 'Information request should have been failed')
