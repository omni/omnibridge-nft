...
async function deployForeign() {
  ...
  const foreignBridgeStorage = await deployContract('foreign', EternalStorageProxy, [])
  ...
  if (!tokenImageERC721) {
    ...
    const image = await deployContract('foreign', ERC721BridgeToken, ['', '', ZERO_ADDRESS])
  ...
  if (!tokenImageERC1155) {
    ...
    const image = await deployContract('foreign', ERC1155BridgeToken, ['', '', ZERO_ADDRESS])
  ...
  const foreignBridgeImplementation = await deployContract('foreign', ForeignNFTOmnibridge, [FOREIGN_TOKEN_NAME_SUFFIX])
